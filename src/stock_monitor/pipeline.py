...
from stock_monitor.models.pillars import compute_shap_ranges
...
def run_training(
    ...
    # Compute SHAP value ranges across training sample for stable 0-100 pillar scores
    try:
        ranges = compute_shap_ranges(model.base, valid[list(model.features)])
        ...
        pass  # SHAP ranges are best-effort
    ...
    if not short_pooled.empty:
        ...
        try:
            short_model = train_calibrated_model(
                valid_short,
                params=SHORT_HORIZON_LGBM_PARAMS,
