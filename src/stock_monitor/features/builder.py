...
def build_training_frame(
    ...
    benchmark's over the same window (or beats by >5% when ``label_mode="excess"``),
    else 0. Both features and label are computed PIT-correctly: no row can see
    data past its own ``as_of``.
